...
export default function BeritaAdmin() {
  ...
  const [formData, setFormData] = useState({ judul: '', slug: '', konten: '', is_published: true });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const dataToInsert = { ...formData, slug: formData.slug || generateSlug(formData.judul) };
      const { error } = await supabase.from('berita').insert([dataToInsert]);
      if (error) throw error;
      setShowModal(false);
      setFormData({ judul: '', slug: '', konten: '', is_published: true });
      fetchBerita();
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex space-x-3">
                ...
                <button type="submit" className="flex-1 px-4 py-3 bg-primary-600 text-white font-bold rounded-xl shadow-lg">Publikasikan</button>
              </div>
            </form>
          </div>
        </div>
